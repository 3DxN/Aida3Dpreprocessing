...
stitched_hne = 255 * np.ones((IMG_HEIGHT_STITCHED+GAP,IMG_HEIGHT_STITCHED+GAP,4),dtype=np.uint8) 
stitched_cd8 = np.zeros((IMG_HEIGHT_STITCHED+GAP,IMG_HEIGHT_STITCHED+GAP,4),dtype=np.uint8) 
stitched_gH2aX = np.zeros((IMG_HEIGHT_STITCHED+GAP,IMG_HEIGHT_STITCHED+GAP,4),dtype=np.uint8)

for row in range(0,MAX_ROW):
    for col in range(0,MAX_COL):
        hne_slice = tifffile.imread(f'pseudoHnE_COL_{col}_ROW_{row}.tif')
        print(row, col, hne_slice.shape)

        c_start = col*(TILE_SIZE+GAP)+GAP
        c_end = (col+1)*(TILE_SIZE+GAP)
        r_start = row*(TILE_SIZE+GAP)+GAP
        r_end = (row+1)*(TILE_SIZE+GAP)
