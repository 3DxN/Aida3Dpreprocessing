...
for idx, c in enumerate(CLASSES):
    ...
    pct_gh2ax[c] = []
    pct_cd8[c] = []
    mean_intensity_gh2ax[c] = []
    gh2ax_ctr = 0
    cd8_ctr = 0

    f = open(c + '_texture_features_w_neighbors.csv', 'w')

    for stack in root.iter('Stack'):
        col, row = int(stack.get('COL')), int(stack.get('ROW'))
        print(f'col: {col}, row: {row}')

        segment_file = 'tile__H%03d_V%03d.tif_CELLPOSE-LABELS_cp_masks.tif' % (col, row)
        raw_file = str(stack.get('IMG_REGEX'))

        ## Segmentation file seems to be the output from the cellpose
        print('segmentation file path: ', os.path.join(SEGMENTATION_PATH[idx], 'Cellpose3DOut', segment_file))

        start_time = time.time()
        segmentation = tifffile.imread(os.path.join(SEGMENTATION_PATH[idx], 'Cellpose3DOut', segment_file))
        elapsed_time = time.time() - start_time
        print(f"Time taken for loading segmentation image: {elapsed_time} seconds")
        print(f'Number of unique cell labels in the segmentation): {len(np.unique(segmentation))}')
        print('')
        ...
        start_time = time.time()
        ims_file = ims(os.path.join(RAW_IMAGE_PATH[idx], raw_file))
        elapsed_time = time.time() - start_time
        print(f"Time taken for ims raw image: {elapsed_time} seconds")

        ims_file.change_resolution_lock(0)
        img = ims_file[0, :, :, :, :]
        print(f'ims_file.shape: {img.shape}')
        print('')
        ...
        elapsed_time = time.time() - start_time
        print(f"Time taken for Generating masks for gh2ax and cd8 masks: {elapsed_time} seconds")
        print('')
        ...
        print("Now calling get_cd8_segmentation_by_dilation")
        start_time = time.time()
        segmentation_cd8 = get_cd8_segmentation_by_dilation(segmentation, cd8_mask)
        elapsed_time = time.time() - start_time
        print(f"Time taken for get_cd8_segmentation_by_dilation: {elapsed_time} seconds")
        print('')

        ## Serial procesisng ROI labels in chunk
        print('Serial processing gH2AX__pixel_count_threshold')
        print(f'Number of unique labels in segmentation_masked: {len(np.unique(segmentation_masked))}')
        start_time = time.time()

        for lbl in tqdm(np.unique(segmentation_masked)):
            if lbl == 0:
                continue
        ...
        elapsed_time = time.time() - start_time
        print(f"Time taken for processing gH2AX__pixel_count_threshold: {elapsed_time} seconds")
        print('')

        ## Processing in parallel in batch
        print('Entering features calculation')
        print(f'Number of unique labels in segmentation_masked: {len(np.unique(segmentation_masked))}')
        start_time = time.time()
        ...
        for lbl in tqdm(unique_labels):
            ...
            mean_intensity_gh2ax[c].append((gh2ax * roi).sum() / np.count_nonzero(roi))
        ...
        print('Starting Parallel processing for Feature Calculation')
        ...
        elapsed_time = time.time() - start_time
        print(f"Time taken for Feature calculation: {elapsed_time} seconds")
        print('')

        # Finding neighbors and writing results to file
        print('Entering Parallel processing for finding neighbors')
        start_time = time.time()
        ...
        elapsed_time = time.time() - start_time
        print(f"Time taken for finding neighbors: {elapsed_time} seconds")
        print('')
        ...
        for (lbl_feature, props), (lbl_neighbor, neighbors) in zip(results, neighbor_results):
            ...
            line = f"{segment_file},{lbl_feature},{','.join(map(str, props))}"
            line += f",{','.join(map(str, neighbors[0]))}\n"
            f.write(line)

        elapsed_time = time.time() - start_time
        print(f"Time taken for saving results: {elapsed_time} seconds")
        print('')

        tifffile.imsave(os.path.join(SEGMENTATION_PATH[idx], 'Cellpose3DOut', segment_file[:-4] + '_gh2ax.tif'), segmentation_gh2ax)
        tifffile.imsave(os.path.join(SEGMENTATION_PATH[idx], 'Cellpose3DOut', segment_file[:-4] + '_cd8.tif'), segmentation_cd8)

        # append to json files
        try:
            with open(os.path.join(SEGMENTATION_PATH[idx], 'Cellpose3DGeometryAndFeatures', 'tile__H%03d_V%03d.tif_CELLPOSE-LABELS_seg.npy.pickle_.json' % (col,row))) as j:
            ...
            with open(os.path.join(SEGMENTATION_PATH[idx], 'Cellpose3DGeometryAndFeatures', 'tile__H%03d_V%03d.tif_CELLPOSE-LABELS_seg.npy.pickle_w_gh2ax.json' % (col,row)), 'w') as j:
                json.dump(json_data, j)
        except FileNotFoundError:
            pass

        pct_cd8[c].append((len(np.unique(segmentation_cd8))-1)/(len(np.unique(segmentation))-1))
        pct_gh2ax[c].append((len(np.unique(segmentation_masked))-1)/(len(np.unique(segmentation))-1))

    # print(gh2ax_sum/gh2ax_ctr)
    f.close()

print('Cell analysis is finished now')
elapsed_time = time.time() - start_time0
print(f"Time taken for Cell analysis with Parallel processing: {elapsed_time} seconds")
print('')
...
for idx,c in enumerate(CLASSES):
    ...
    for stack in root.iter('Stack'):
        ...
        cd8_segment_file = 'tile__H%03d_V%03d.tif_CELLPOSE-LABELS_cp_masks_cd8.tif' % (col,row)
        gh2ax_segment_file = 'tile__H%03d_V%03d.tif_CELLPOSE-LABELS_cp_masks_gh2ax.tif' % (col,row)
        nuclei_segment_file = 'tile__H%03d_V%03d.tif_CELLPOSE-LABELS_cp_masks.tif' % (col,row)

        cd8_segmentation = tifffile.imread(os.path.join(SEGMENTATION_PATH[idx], 'Cellpose3DOut', cd8_segment_file))
        gh2ax_segmentation = tifffile.imread(os.path.join(SEGMENTATION_PATH[idx], 'Cellpose3DOut', gh2ax_segment_file))
        nuclei_segmentation = tifffile.imread(os.path.join(SEGMENTATION_PATH[idx], 'Cellpose3DOut', nuclei_segment_file))
