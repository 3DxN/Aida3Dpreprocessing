...
def main():
    ...
    parser.add_argument('--TIFFwriteout',  metavar=('target dir/filename prefix', 'zAnisotropyTarget'), 
            ...
            default = ['/TIFFtiles/', 2.0]) # # Add sanity check for dir and zAnisotropy?
    ...
    if not args.computeMaskOnly:     
        ...
        for tileIdx, tile in enumerate(imarisTeraStitcherMetaData_root.iter('Stack')):
            ...
            temp_HnE_out_filename = f"pseudoHnE_COL_{ta['COL']}_ROW_{ta['ROW']}.tif"
        ...
        imsave(args.outfileHNEmaxIntensity.name, HnEimageTilesMaxProj, photometric='rgb')
        imsave(args.outfileNUCLsubstackMaxIntensity.name, NuclTilesMaxProj, photometric='minisblack')
        imsave(args.outfileCYTOsubstackMaxIntensity.name, CytoTilesMaxProj, photometric='minisblack')

    imsave(args.maskFile.name, maskTiles, photometric='minisblack')
